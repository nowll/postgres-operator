...
kind-load: docker-build ## Load docker image into kind cluster
	kind load docker-image ${IMG} --name postgres-operator

.PHONY: dev-deploy
dev-deploy: kind-load deploy ## Build, load to kind, and deploy
...
.PHONY: samples
samples: ## Apply sample PostgresInstance resources
